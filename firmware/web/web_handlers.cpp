...
void handleSetCurrent()
{
    if (!intervalometer.intervalometerActive)
    {
        ...
        // Validate and set capture mode
        int modeInt = server.arg(CAPTURE_MODE).toInt();
        if (!validateCaptureMode(modeInt, intervalometer.currentErrorMessage))
        {
            server.send(200, MIME_TYPE_TEXT,
                        languageErrorMessageStrings[language][intervalometer.currentErrorMessage]);
            return;
        }
        intervalometer.currentSettings.mode = static_cast<Intervalometer::Mode>(modeInt);

        // Validate and set exposure time
        int exposureTime = server.arg(EXPOSURE_TIME).toInt();
        if (!validateExposureTime(exposureTime, intervalometer.currentErrorMessage))
        {
            server.send(200, MIME_TYPE_TEXT,
                        languageErrorMessageStrings[language][intervalometer.currentErrorMessage]);
            return;
        ...
        // Validate and set exposures
        int exposures = server.arg(EXPOSURES).toInt();
        if (!validateExposures(exposures, intervalometer.currentErrorMessage))
        {
            server.send(200, MIME_TYPE_TEXT,
                        languageErrorMessageStrings[language][intervalometer.currentErrorMessage]);
            return;
        ...
        // Validate and set pre-delay
        int preDelay = server.arg(PREDELAY).toInt();
        if (!validatePreDelay(preDelay, intervalometer.currentErrorMessage))
        {
            server.send(200, MIME_TYPE_TEXT,
                        languageErrorMessageStrings[language][intervalometer.currentErrorMessage]);
            return;
        ...
        if (preDelay == 0)
            preDelay = 5; // Default value
        intervalometer.currentSettings.preDelay = preDelay;

        // Validate and set delay time
        int delayTime = server.arg(DELAY).toInt();
        if (!validateDelayTime(delayTime, intervalometer.currentErrorMessage))
        {
            server.send(200, MIME_TYPE_TEXT,
                        languageErrorMessageStrings[language][intervalometer.currentErrorMessage]);
            return;
        ...
        if (!validateFrames(frames, intervalometer.currentErrorMessage))
        {
            server.send(200, MIME_TYPE_TEXT,
                        languageErrorMessageStrings[language][intervalometer.currentErrorMessage]);
            return;
        ...
        // Validate and set pan angle
        float panAngle = server.arg(PAN_ANGLE).toFloat() / 100;
        if (!validatePanAngle(panAngle, intervalometer.currentErrorMessage))
        {
            server.send(200, MIME_TYPE_TEXT,
                        languageErrorMessageStrings[language][intervalometer.currentErrorMessage]);
            return;
        ...
        // Validate and set pan direction
        int panDirection = server.arg(PAN_DIRECTION).toInt();
        if (!validatePanDirection(panDirection, intervalometer.currentErrorMessage))
        {
            server.send(200, MIME_TYPE_TEXT,
                        languageErrorMessageStrings[language][intervalometer.currentErrorMessage]);
            return;
        ...
        // Validate and set enable tracking
        int enableTracking = server.arg(ENABLE_TRACKING).toInt();
        if (!validateEnableTracking(enableTracking, intervalometer.currentErrorMessage))
        {
            server.send(200, MIME_TYPE_TEXT,
                        languageErrorMessageStrings[language][intervalometer.currentErrorMessage]);
            return;
        ...
        // Validate and set dither choice
        int dither = server.arg(DITHER_CHOICE).toInt();
        if (!validateDitherChoice(dither, intervalometer.currentErrorMessage))
        {
            server.send(200, MIME_TYPE_TEXT,
                        languageErrorMessageStrings[language][intervalometer.currentErrorMessage]);
            return;
        ...
        // Validate and set dither frequency
        int ditherFrequency = server.arg(DITHER_FREQUENCY).toInt();
        if (!validateDitherFrequency(ditherFrequency, intervalometer.currentErrorMessage))
        {
            server.send(200, MIME_TYPE_TEXT,
                        languageErrorMessageStrings[language][intervalometer.currentErrorMessage]);
            return;
        ...
        if (!validateFocalLength(focalLength, intervalometer.currentErrorMessage))
        {
            server.send(200, MIME_TYPE_TEXT,
                        languageErrorMessageStrings[language][intervalometer.currentErrorMessage]);
            return;
        ...
        // Validate and set pixel size
        float pixelSize = server.arg(PIXEL_SIZE).toFloat() / 100;
        if (!validatePixelSize(pixelSize, intervalometer.currentErrorMessage))
        {
            server.send(200, MIME_TYPE_TEXT,
                        languageErrorMessageStrings[language][intervalometer.currentErrorMessage]);
            return;
